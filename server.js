...
const https = require("https"); // Dùng cho API Geocode (hỗ trợ mọi bản Node)
require("dotenv").config();

const uploadRoute = require("./upload.route"); // Import route xử lý upload ảnh Cloudinary
...
app.use(express.json({ limit: "50mb" })); // Tăng giới hạn payload để có thể nhận ảnh Base64
...
app.use("/api/upload", uploadRoute); // Cấu hình route /api/upload
...
app.post("/api/forgot-password", async (req, res) => {
    ...
    try {
        ...
        if (user) {
            res.json({
                success: true,
                message: "Link khôi phục đã gửi đến " + email,
            });
        } else {
            res
                .status(404)
...
// Nhận phản hồi từ Feedback (Tính năng đang bị thiếu API)
app.post("/api/feedback", async (req, res) => {
    try {
        console.log("💌 Nhận phản hồi mới:", req.body);
        // Tạm thời trả về OK để giao diện không bị báo lỗi 404
...
app.post("/api/rescuemap", async (req, res) => {
    try {
        ...
        // Sửa lỗi thiếu id: Trả về _id để script1.js có thể chạy tiếp vòng lặp vá ảnh và địa chỉ
...
// Cập nhật địa chỉ cho bản ghi cứu hộ (Reverse Geocoding lưu lại)
...
// Cập nhật link ảnh (từ Cloudinary) cho bản ghi cứu hộ
...
// Proxy vượt rào CORS cho Nominatim API (Geocode)
app.get("/api/geocode", async (req, res) => {
    try {
        ...
        // Thay fetch bằng https native để không bị lỗi trên các bản Node.js đời cũ (<18)
